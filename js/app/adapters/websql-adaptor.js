app.adapters.Database = (function () {
  console.log("Loading websql-storage adapter module");

  this.initialize = function () {
    var deferred = $.Deferred();
    this.db = window.openDatabase("UserDB", "1.0", "user Demo DB", 200000);
    this.db.transaction(
      function (tx) {
        createTable(tx);
        addSampleData(tx);
    ...
    this.db.transaction(
      function (tx) {
        ...
        var sql = "SELECT e.id, e.firstName, e.lastName, e.title, e.city, e.officePhone, e.cellPhone, e.email, e.pic, e.managerId, m.firstName managerFirstName, m.lastName managerLastName, count(r.id) reportCount " +
          "FROM user e " +
          "LEFT JOIN user r ON r.managerId = e.id " +
